...
const AmountWithdrawal = ({ transaction }: { transaction?: Transaction }) => {
  ...
  const defaultValues = useMemo(() => ({
    ...
    description: transaction
      ? "Updated Amount Withdrawal Details"
      : "Amount Withdrawal Details",
  ...
  const addWithdrawalForm = useForm<z.infer<typeof addInvestmentSchema>>({
  ...
  const onSubmit = useCallback((values: z.infer<typeof addInvestmentSchema>) => {
    const { amount, description, paymentMethod, date, selectedPartnerId } =
      values;
    const partner = members.find((member) => member.id === selectedPartnerId);
    if (transaction) {
      dispatch(
        updateTransactionEntry({
          id: transaction.id,
          ...
          amount,
          description: description || "Updated Amount Withdrawal Details",
          date: date,
          paymentMethod: paymentMethod as PaymentMethod,
          incomeOrExpenseType: IncomeOrExpenseType.WITHDRAWAL,
          transactionType: TransactionType.WITHDRAWAL,
          memberName: partner?.name,
          memberID: selectedPartnerId,
    ...
    dispatch(
      createNewtransactionEntry({
        amount,
        description: description || "Amount Withdrawal Details",
        paymentMethod: paymentMethod as PaymentMethod,
        date,
        incomeOrExpenseType: IncomeOrExpenseType.WITHDRAWAL,
        transactionType: TransactionType.WITHDRAWAL,
      ...
      {step === 1 ? (
        <>
          <DialogHeader>
            ...
            <DialogDescription>
              Please enter the details of an amount withdrawal by a partner
            </DialogDescription>
          </DialogHeader>
          <Form {...addWithdrawalForm}>
            <form
              onSubmit={addWithdrawalForm.handleSubmit(onSubmit)}
              className="space-y-6"
            >
              <FormInput
                control={addWithdrawalForm.control}
                label="Amount"
                name="amount"
              ...
              <GetMembersList control={addWithdrawalForm.control} />
              <DialogFooter>
                {status === "creating" || status === "updating" ? (
                  <Button disabled>
                    ...
                    <Button type="submit">
                    {transaction ? "Update Withdrawal" : "Add Withdrawal"}
                    </Button>
                )}
              </DialogFooter>
            </form>
