...
import { createClosingEntry } from "../state/closing/closingOperations";
import {
  Admin,
  CashAccount,
  Circle,
...
export const getDailySheetData = () => {
  try {
    ...
    const closingBF = (data.cashAccount?.closingBalance || 0) + totalLeft;
    ...
    return {
      ...
      openingBF: data.cashAccount?.closingBalance,
...
export const createDailySheet = async () => {
  try {
    const transactionData = getTransactionData();
    const dailySheetData = getDailySheetData();

    if (!transactionData.admin || !transactionData.selectedCircle) {
      throw new Error("Missing required admin or circle data");
    }

    if (!transactionData.range || transactionData.range.length !== 2) {
      throw new Error("Invalid date range");
    }

    // if (
    //   transactionData.outstandingAmountNow !==
    //   dailySheetData.closingTotalOutstanding
    // ) {
    //   throw new Error("Outstanding amount are not Equal");
    // }

    const closingEntryData: VerifyClosingTransactionMutationVariables = {
      adminId: transactionData.admin.id,
      adminEmailId: transactionData.admin.emailId,
      circleId: transactionData.selectedCircle.id,
      openingBalance: dailySheetData.openingBF ?? 0,
      openingEntryDate: transactionData.range[0],
      closingBalance: dailySheetData.closingBF,
      closingEntryDate: transactionData.range[1],
      description: `Closing Entry for ${formatYYYYDDMMToISOString(
        transactionData.range[1]
      )}`,
      loansGiven: transactionData.newLoanPayments,
      repayments: transactionData.collectionAmount,
      interest: transactionData.interest,
      cashflowIn: transactionData.cashflowIn,
      cashflowOut: transactionData.cashflowOut,
      outstandingAmount: dailySheetData.closingTotalOutstanding,
      writeOff: transactionData.writeOff,
      businessLoss: transactionData.businessLoss,
      existingLoanOutstanding: transactionData.existingLoanOutstanding,
    };

    await store.dispatch(createClosingEntry({ ...closingEntryData }));
    return closingEntryData;
  } catch (error) {
    console.error("Error in createDailySheet:", error);
    throw error;
  }
};
